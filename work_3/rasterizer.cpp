...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        Eigen::Matrix4f inv_trans = (view * model).inverse().transpose(); //在view space下计算法线
        Eigen::Vector4f n[] = {
        ...
        for (int i = 0; i < 3; ++i)
        {
			...
			newtri.setVertex(i, v[i]); //三角形中顶点此时存放的是屏幕空间坐标
        }

        for (int i = 0; i < 3; ++i)
        {
            //view space normal
			newtri.setNormal(i, n[i].head<3>()); //view space下的法线
        }
		...
		//此时传送到该函数的是三角形三个顶点在相机坐标系下的坐标，同时v[3]中存放的是屏幕空间坐标  
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for (int i = minX; i <= maxX; ++i) {
        for (int j = minY; j <= maxY; ++j) {
            ...
            if (!insideTriangle(pixel_x,pixel_y,t.v)) continue;
            ...
            if (depth_buf[get_index(i, j)] <= zp) continue;
			...
			auto interpolated_shadingcoords = interpolate(alpha, beta, gamma, view_pos[0], view_pos[1], view_pos[2], Z); //view_pos
